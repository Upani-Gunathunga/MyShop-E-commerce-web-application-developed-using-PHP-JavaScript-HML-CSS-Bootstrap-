...
function showPassword3() {
    ...
    var pwb = document.getElementById("basic-addon2");

    if (pw.type == "password") {
        pw.type = "text";

        pwb.innerHTML = '<i class="bi bi-eye-slash"></i>';

    } else {
        pw.type = "password";
        pwb.innerHTML = '<i class="bi bi-eye"></i>';
...
function updateProfile() {
    ...
    //  alert(profile_img.files[0]);
    //  alert(profile_img.files[0].name);
    ...
    r.onreadystatechange = function () {
        if (r.status == 200 && r.readyState == 4) {
            ...
            if (t == "success") {
                // window.Location="home.php";
                alert("Your profile has been updated. Reload the page and sign in to view your updated profile.");
                signout();
            } else {
                alert(t);
            }
...
function loadDistricts() {
    ...
    r.open("GET", "loadDistrictProcess.php?p=" + province, true);
    r.send();
...
function loadCities() {
    ...
    r.open("GET", "loadCityProcess.php?d=" + district, true);
    r.send();
}
...
function addNewClr() {
    var nclr = document.getElementById("clr_in").value;
    var r = new XMLHttpRequest();
    ...
    r.onreadystatechange = function () {
        if (r.status == 200 && r.readyState == 4) {
            var t = r.responseText;
            if (t == "success") {
                location.reload("clr_in");
                alert("successfully saved the color.");
            } else {
                alert(t);
            }
        }
    ...
    r.open("GET", "addNewClrProcess.php?nclr=" + nclr, true);
    r.send();
...
function advancedSearch2(x) {
    ...
    r.open("POST", "advancedSearchProcess2.php", true);
    r.send(f);
    ...
    r.onreadystatechange = function () {
        if (r.status == 200 && r.readyState == 4) {
            ...
            if (t == "Added") {
                ...
                alert("Product removed to the watchlist successfully");
                window.location.reload();
            }
            alert(t);
        }
...
function addToCart(id) {
    ...
    r.onreadystatechange = function () {
        if (r.status == 200 && r.readyState == 4) {
            ...
            if (t == "This product already exists") {
    ...
    r.onreadystatechange = function () {
        if (r.status == 200 && r.readyState == 4) {
            ...
            if (t == "address error") {
                ...
                var payment = {
                    "sandbox": true,
                    "merchant_id": "1224144",    // Replace your Merchant ID
                    "return_url": "http://localhost/myshopproject/singleProductView.php?id=" + pid,     // Important
                    "cancel_url": "http://localhost/myshopproject/singleProductView.php?id=" + pid,     // Important
...
//not done in class // check one by one
...
function addFeedback(id){
    ...
    // var m = new bootstrap.Modal(document.getElementById('feedbackModal'+id), {})
    // m.show();
