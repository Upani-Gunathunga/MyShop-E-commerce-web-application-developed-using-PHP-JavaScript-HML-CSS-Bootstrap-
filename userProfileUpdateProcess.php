...
`users_email`='" . $user_email . "' ");
    ...
    `line2`='" . $line2 . "',`postal_code`='" . $pc . "',`city_city_id`='" . $city . "' WHERE `users_email`='" . $user_email . "' ");
    } else {
    ...
    if (isset($_FILES["img"])) {
        ...
        if (in_array($file_type, $allowed_image_extensions)) {
            ...
            echo ("File type is not allowed to upload");
        }
    } else {
    ...
    if ($user_num == 1) {
        ...
        Database::iud("UPDATE `users` SET `fname`='" . $fname . "',`lname`='" . $lname . "',`email`='" . $email . "',`password`='" . $pw . "',
